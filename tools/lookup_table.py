...
def compute_steps_to_standstill(table, angular_velocity_grid, angle_of_attack_grid, single_step_stabilizable):
    ...
    while still_updating:
        ...
        for i, angular_velocity in enumerate(angular_velocity_grid):
            ...
            candidate_costs = np.full(len(angle_of_attack_grid), np.inf)
            ...
            if np.all(np.isinf(candidate_costs)):
                continue

            best_action_index = np.argmin(candidate_costs)
            steps_to_standstill[i] = candidate_costs[best_action_index]
            best_angle_of_attack[i] = angle_of_attack_grid[best_action_index]
            still_updating = True
