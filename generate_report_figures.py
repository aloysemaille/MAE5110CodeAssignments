...
from tools.trajectory_plots import (
...
def test_grid_resolution(resolutions, params, timestep, test_velocities):
    ...
    for n_velocity, n_angle in resolutions:
        froude_two_velocity = np.sqrt(
            2 * params["gravity"] / params["length"]
        )

        velocity_grid = np.linspace(
            0.0, froude_two_velocity, n_velocity
        )

        angle_grid = np.linspace(
            angle_of_attack_min, angle_of_attack_max, n_angle
        )

        table = build_lookup_table(
            velocity_grid, angle_grid, params, timestep
        )

        single_step, single_step_angle = find_single_step_stabilizable(
            table,
            velocity_grid,
            angle_grid,
            theta_points,
            angular_velocity_points,
            grid_result,
        )

        steps, _ = compute_fastest_steps_to_standstill(
            table,
            velocity_grid,
            angle_grid,
            single_step,
        ...
        outcomes = [
            steps[np.argmin(np.abs(velocity_grid - velocity))]
...
test_velocities = [1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0]
...
angular_velocity_grid = np.linspace(
    0.0, froude_two_velocity, 40
)

angle_of_attack_grid = np.linspace(
    angle_of_attack_min, angle_of_attack_max, 20
)

table = build_lookup_table(
    angular_velocity_grid,
    angle_of_attack_grid,
    params,
    lookup_timestep,
)
...
fastest_steps_to_standstill, fastest_angle_of_attack = (
    compute_fastest_steps_to_standstill(
        table,
        angular_velocity_grid,
        angle_of_attack_grid,
        single_step_stabilizable,
        single_step_angle_of_attack,
    )
)

slowest_steps_to_standstill, slowest_angle_of_attack = (
    compute_slowest_steps_to_standstill(
        table,
        angular_velocity_grid,
        angle_of_attack_grid,
...
example_initial_state = np.array([0.0, 3.52])
...
plot_state_space_trajectories(
    fastest_state_history,
    fastest_completed_steps,
    slowest_state_history,
    slowest_completed_steps,
    example_initial_state,
    save_path="images/state_space_trajectory.png",
)

plot_steps_to_standstill(
    angular_velocity_grid,
    fastest_steps_to_standstill,
    save_path="images/steps_to_standstill.png",
)
